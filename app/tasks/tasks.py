...
class GetAd(object):
    ...
    def checkTextContains(self, driver):
        try:
            driver.find_element(
                By.XPATH, "//*[contains(text(),'Такой страницы нe существует')]"
            )
    ...
    def start(self):
        ...
        with self.get_response(request) as response:
            ...
            try:
                x = driver.find_element(By.XPATH, self.xpaths.to_coord).get_attribute(
                    "data-map-lat"
                )
                y = driver.find_element(By.XPATH, self.xpaths.to_coord).get_attribute(
                    "data-map-lon"
                )
            except:
